...
class EmbeddingGate:
    ...
    def judge_candidate_embedding(self, url: str, data_uri: str) -> dict[str, Any] | None:
        """
        嵌入候选并与已采纳注册表比对。

        命中重复返回审计记录（verdict="duplicate"，含 duplicate_of 与 cos）；
        未命中、未注册或嵌入失败（fail-open）返回 None 放行。命中时同样
        写缓存，register_accepted 只对真正被采纳的 URL 生效。
        """
