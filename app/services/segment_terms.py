...
在分段之后、素材搜索之前，先用 LLM 把每个片段提炼成最多 3 个英文
搜索词（1 个主词 + 2 个备用词，各自 1-3 个单词）——沿用 `generate_terms`
的提示词结构（模型对该格式已稳定），但把 Context 换成单个片段并绑定主题词。
主词搜索失败时素材层用备用词重试，替代原先的邻近片段回退。
...
# 每段提炼的候选词数量；第一个词为主搜索词，其余为主词搜索失败时的
# 备用词。备用词必须覆盖同一时刻的不同画面角度，避免近似词命中同一
# 素材候选池（见 _build_batch_prompt 的多样性约束）。
TERMS_PER_SEGMENT = 3

_MAX_RETRIES = 2

# 字符级 CJK 判定：主题词若含 CJK 字符则不追加到搜索词。
# Pexels/Pixabay/Coverr 对中文查询召回极差且不稳定（UAT 审计确认），搜索
# 词必须是纯英文。片段原文回退同理——含 CJK 的原文不应作为搜索词。
...
2. each search term should consist of 1-3 words, always translate the main subject of the video into English and append it.
3. you must only return the json-array of strings. you must not return anything else. you must not return the script.
4. the search terms must be related to what the segment describes, not just the subject.
5. every search term must be pure English (A-Z letters and spaces only). never include any Chinese or other non-English characters, even if the subject or the narration is written in Chinese.
...
Please note that you must use English for generating video search terms; Chinese is not accepted. If the subject or a segment is written in Chinese, translate its meaning into English search terms.
""".strip()
...
3. each search term should consist of 1-3 words, always translate the main subject of the video into English and append it.
4. each search term must describe what that segment's narration says (its visual moment), not the whole video.
5. every search term must be pure English (A-Z letters and spaces only). never include any Chinese or other non-English characters, even if the subject or the narration is written in Chinese.
6. the {per_segment} terms for the same segment must describe DIFFERENT visual angles, scenes, or shot types of that segment's moment — never synonyms or minor rephrasings of each other, because near-duplicate terms return the same stock-footage candidate pool.
...
def extract_terms_for_segments(
    ...
    Args:
        ...
        amount: 每段生成的词数；第一个词为主搜索词，其余为备用词。
        batch_size: 一次 LLM 请求覆盖的分段数。

    Returns:
        {segment_index: [search_term, ...]}，词条有序、主词在前，每段
        1..amount 个。某段持续失败或全部词条被过滤时该段不出现在
        结果里，由素材搜索层的回退链兜底。
    """
    ...
    for offset in range(0, len(pending), max(1, int(batch_size))):
        ...
        for position, (_, index, _) in enumerate(numbered):
            # 搜索 API 仅接受英文：最终搜索词 = LLM 词条 + 主题词，两者都
            # 不能带 CJK。主题词是中文时直接放弃追加（没有可追加的英文主题），
            # 让素材层走下一级英文回退，而不是发出必然低召回的混合查询。
            append_subject = bool(video_subject)
            if video_subject and _contains_cjk(str(video_subject)):
                logger.warning(
                    "video subject contains CJK, appending it would break "
                    f"English-only search: index={index}, subject={video_subject!r}"
                )
                append_subject = False
            # 逐词过滤：单个词条含 CJK 只丢弃该词，分段靠剩余词条存活；
            # 全部词条被过滤时分段不进入结果，交给素材层回退链兜底。
            final_terms: List[str] = []
            for term in parsed[
                position * per_segment : (position + 1) * per_segment
            ]:
                final_term = f"{term} {video_subject}".strip() if append_subject else term
                if _contains_cjk(final_term):
                    logger.warning(
                        f"segment term contains CJK, discarding: index={index}, "
                        f"term={final_term!r}"
                    )
                    continue
                final_terms.append(final_term)
            if final_terms:
                terms[index] = final_terms
